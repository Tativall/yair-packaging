<?php
// =====================================================
// config/supabase.php — Credenciales Yair Packaging
// =====================================================
...
function uploadToSupabase(array $file): string {
    ...
    if (empty($data['Key']) && empty($data['key'])) throw new Exception('Error al subir imagen');
...
function loginAdmin(): void {
    $t = time();
    $s = hash_hmac('sha256', ADMIN_USER.':'.$t, AUTH_SECRET);
    setcookie(AUTH_COOKIE, ADMIN_USER.':'.$t.':'.$s, [
        'expires'  => time()+86400,
        'path'     => '/',
        'httponly' => true,
        'samesite' => 'Lax',
