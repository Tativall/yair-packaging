...
switch ($action) {
    ...
    case 'update':
        $id     = (int)($_POST['id'] ?? 0);
        $nombre = trim($_POST['nombre'] ?? '');
        if (!$id || !$nombre) jsonResponse(['error' => 'Datos inválidos — id: ' . ($_POST['id']??'vacío')], 400);

        $sc = $db->prepare("SELECT id FROM categorias WHERE nombre = ?");
        $sc->execute([trim($_POST['categoria'] ?? '')]);
        $catId = ($sc->fetch())['id'] ?? null;
        ...
        if (!empty($_FILES['foto']['name']) && $_FILES['foto']['error'] === 0) {
            $foto = handleUpload($_FILES['foto']);
            if ($fotoActual && file_exists(__DIR__ . '/../assets/uploads/' . $fotoActual)) {
                unlink(__DIR__ . '/../assets/uploads/' . $fotoActual);
            }
        }
    ...
    case 'delete':
        ...
        $stmt->execute([$id]);
        $p = $stmt->fetch();
        if ($p && $p['foto'] && file_exists(__DIR__ . '/../assets/uploads/' . $p['foto'])) {
            unlink(__DIR__ . '/../assets/uploads/' . $p['foto']);
        }
    ...
    case 'change_password':
        ...
        $db->prepare("INSERT OR REPLACE INTO settings (clave, valor) VALUES ('admin_password', ?)")
