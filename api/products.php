...
if (!in_array($action, $readOnly) && !isAdminLoggedIn()) {
...
switch ($action) {
    ...
    case 'get':
        $id = (int)($_GET['id']??0);
        $s  = $db->prepare("SELECT p.*,c.nombre as categoria FROM productos p LEFT JOIN categorias c ON p.categoria_id=c.id WHERE p.id=?");
        $s->execute([$id]);
        $p = $s->fetch();
    ...
    case 'create':
        ...
        $sc = $db->prepare("SELECT id FROM categorias WHERE nombre=?"); $sc->execute([$cat]);
        $catId = ($sc->fetch())['id'] ?? null;
        $foto = '';
        if (!empty($_FILES['foto']['name'])&&$_FILES['foto']['error']===0) $foto=handleUpload($_FILES['foto']);
        $s = $db->prepare("INSERT INTO productos (nombre,descripcion,categoria_id,precio,unidad,medidas,etiqueta,emoji,foto) VALUES (?,?,?,?,?,?,?,?,?)");
        $s->execute([$nombre,trim($_POST['descripcion']??''),$catId,(float)($_POST['precio']??0),trim($_POST['unidad']??'unid'),trim($_POST['medidas']??''),trim($_POST['etiqueta']??''),trim($_POST['emoji']??'📦'),$foto]);
        jsonResponse(['success'=>true,'id'=>$db->lastInsertId(),'message'=>'Producto creado']);
        break;

    case 'update':
        $id=$db=(int)($_POST['id']??0); $id=(int)($_POST['id']??0);
        $nombre=trim($_POST['nombre']??'');
        if(!$id||!$nombre) jsonResponse(['error'=>'Datos inválidos'],400);
        $sc=$db->prepare("SELECT id FROM categorias WHERE nombre=?"); $sc->execute([trim($_POST['categoria']??'')]);
        $catId=($sc->fetch())['id']??null;
        $fotoActual=trim($_POST['foto_actual']??''); $foto=$fotoActual;
        ...
        $s=$db->prepare("UPDATE productos SET nombre=?,descripcion=?,categoria_id=?,precio=?,unidad=?,medidas=?,etiqueta=?,emoji=?,foto=?,updated_at=datetime('now') WHERE id=?");
        $s->execute([$nombre,trim($_POST['descripcion']??''),$catId,(float)($_POST['precio']??0),trim($_POST['unidad']??'unid'),trim($_POST['medidas']??''),trim($_POST['etiqueta']??''),trim($_POST['emoji']??'📦'),$foto,$id]);
        jsonResponse(['success'=>true,'message'=>'Actualizado']);
        break;

    case 'delete':
        $id=(int)($_GET['id']??0);
        $s=$db->prepare("SELECT foto FROM productos WHERE id=?"); $s->execute([$id]);
        $p=$s->fetch();
    ...
    case 'save_settings':
        ...
        $s=$db->prepare("INSERT OR REPLACE INTO settings (clave,valor) VALUES (?,?)");
        foreach($allowed as $k) if(isset($input[$k])) $s->execute([$k,trim($input[$k])]);
...
function handleUpload($file){
    ...
    if(!move_uploaded_file($file['tmp_name'],$dir.$name)) throw new Exception('Error al guardar');
    return $name;
}
